#!/usr/bin/env python3
"""
Validation script for lemkin-integrity
"""
...
def check_module_import():
    ...
    try:
        import lemkin_integrity
        print(f"✅ Module lemkin-integrity imported successfully")
        print(f"   Version: {lemkin_integrity.__version__}")
        return True
    except ImportError as e:
        print(f"❌ Failed to import lemkin-integrity: {e}")
        return False

def check_cli_available():
    """Check if CLI is available"""
    try:
        result = subprocess.run(["lemkin-integrity", "--version"],
                              capture_output=True, text=True)
        if result.returncode == 0:
            print(f"✅ CLI lemkin-integrity available")
            print(f"   Output: {result.stdout.strip()}")
            return True
        else:
            print(f"❌ CLI lemkin-integrity failed with code {result.returncode}")
            return False
    except FileNotFoundError:
        print(f"❌ CLI lemkin-integrity not found")
        return False
...
def main():
    ...
    print("🔍 Validating Integrity installation...")
    print("=" * 50)
    ...
    if passed == total:
        print("🎉 All validation checks passed! Integrity is ready to use.")
        return 0
    else:
