#!/usr/bin/env python3
"""
Validation script for lemkin-geo
"""
...
def check_module_import():
    ...
    try:
        import lemkin_geo
        print(f"✅ Module lemkin-geo imported successfully")
        print(f"   Version: {lemkin_geo.__version__}")
        return True
    except ImportError as e:
        print(f"❌ Failed to import lemkin-geo: {e}")
        return False

def check_cli_available():
    """Check if CLI is available"""
    try:
        result = subprocess.run(["lemkin-geo", "--version"],
                              capture_output=True, text=True)
        if result.returncode == 0:
            print(f"✅ CLI lemkin-geo available")
            print(f"   Output: {result.stdout.strip()}")
            return True
        else:
            print(f"❌ CLI lemkin-geo failed with code {result.returncode}")
            return False
    except FileNotFoundError:
        print(f"❌ CLI lemkin-geo not found")
        return False
...
def main():
    ...
    print("🔍 Validating Geo installation...")
    print("=" * 50)
    ...
    if passed == total:
        print("🎉 All validation checks passed! Geo is ready to use.")
        return 0
    else:
