#!/usr/bin/env python3
"""
Validation script for lemkin-forensics
"""
...
def check_module_import():
    ...
    try:
        import lemkin_forensics
        print(f"✅ Module lemkin-forensics imported successfully")
        print(f"   Version: {lemkin_forensics.__version__}")
        return True
    except ImportError as e:
        print(f"❌ Failed to import lemkin-forensics: {e}")
        return False

def check_cli_available():
    """Check if CLI is available"""
    try:
        result = subprocess.run(["lemkin-forensics", "--version"],
                              capture_output=True, text=True)
        if result.returncode == 0:
            print(f"✅ CLI lemkin-forensics available")
            print(f"   Output: {result.stdout.strip()}")
            return True
        else:
            print(f"❌ CLI lemkin-forensics failed with code {result.returncode}")
            return False
    except FileNotFoundError:
        print(f"❌ CLI lemkin-forensics not found")
        return False
...
def main():
    ...
    print("🔍 Validating Forensics installation...")
    print("=" * 50)
    ...
    if passed == total:
        print("🎉 All validation checks passed! Forensics is ready to use.")
        return 0
    else:
