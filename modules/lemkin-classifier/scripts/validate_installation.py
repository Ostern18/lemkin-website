#!/usr/bin/env python3
"""
Validation script for lemkin-classifier
"""
...
def check_module_import():
    ...
    try:
        import lemkin_classifier
        print(f"✅ Module lemkin-classifier imported successfully")
        print(f"   Version: {lemkin_classifier.__version__}")
        return True
    except ImportError as e:
        print(f"❌ Failed to import lemkin-classifier: {e}")
        return False

def check_cli_available():
    """Check if CLI is available"""
    try:
        result = subprocess.run(["lemkin-classifier", "--version"],
                              capture_output=True, text=True)
        if result.returncode == 0:
            print(f"✅ CLI lemkin-classifier available")
            print(f"   Output: {result.stdout.strip()}")
            return True
        else:
            print(f"❌ CLI lemkin-classifier failed with code {result.returncode}")
            return False
    except FileNotFoundError:
        print(f"❌ CLI lemkin-classifier not found")
        return False
...
def main():
    ...
    print("🔍 Validating Classifier installation...")
    print("=" * 50)
    ...
    if passed == total:
        print("🎉 All validation checks passed! Classifier is ready to use.")
        return 0
    else:
