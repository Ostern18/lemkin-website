#!/usr/bin/env python3
"""
Validation script for lemkin-export
"""
...
def check_module_import():
    ...
    try:
        import lemkin_export
        print(f"✅ Module lemkin-export imported successfully")
        print(f"   Version: {lemkin_export.__version__}")
        return True
    except ImportError as e:
        print(f"❌ Failed to import lemkin-export: {e}")
        return False

def check_cli_available():
    """Check if CLI is available"""
    try:
        result = subprocess.run(["lemkin-export", "--version"],
                              capture_output=True, text=True)
        if result.returncode == 0:
            print(f"✅ CLI lemkin-export available")
            print(f"   Output: {result.stdout.strip()}")
            return True
        else:
            print(f"❌ CLI lemkin-export failed with code {result.returncode}")
            return False
    except FileNotFoundError:
        print(f"❌ CLI lemkin-export not found")
        return False

def check_core_functionality():
    """Check core functionality"""
    try:
        from lemkin_export.core import *
...
def main():
    ...
    print("🔍 Validating Export installation...")
    print("=" * 50)
    ...
    if passed == total:
        print("🎉 All validation checks passed! Export is ready to use.")
        return 0
    else:
