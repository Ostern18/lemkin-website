#!/usr/bin/env python3
"""
Validation script for lemkin-reports
"""
...
def check_module_import():
    ...
    try:
        import lemkin_reports
        print(f"✅ Module lemkin-reports imported successfully")
        print(f"   Version: {lemkin_reports.__version__}")
        return True
    except ImportError as e:
        print(f"❌ Failed to import lemkin-reports: {e}")
        return False

def check_cli_available():
    """Check if CLI is available"""
    try:
        result = subprocess.run(["lemkin-reports", "--version"],
                              capture_output=True, text=True)
        if result.returncode == 0:
            print(f"✅ CLI lemkin-reports available")
            print(f"   Output: {result.stdout.strip()}")
            return True
        else:
            print(f"❌ CLI lemkin-reports failed with code {result.returncode}")
            return False
    except FileNotFoundError:
        print(f"❌ CLI lemkin-reports not found")
        return False

def check_core_functionality():
    """Check core functionality"""
    try:
        from lemkin_reports.core import *
...
def main():
    ...
    print("🔍 Validating Reports installation...")
    print("=" * 50)
    ...
    if passed == total:
        print("🎉 All validation checks passed! Reports is ready to use.")
        return 0
    else:
