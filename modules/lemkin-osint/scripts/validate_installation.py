#!/usr/bin/env python3
"""
Validation script for lemkin-osint
"""
...
def check_module_import():
    ...
    try:
        import lemkin_osint
        print(f"✅ Module lemkin-osint imported successfully")
        print(f"   Version: {lemkin_osint.__version__}")
        return True
    except ImportError as e:
        print(f"❌ Failed to import lemkin-osint: {e}")
        return False

def check_cli_available():
    """Check if CLI is available"""
    try:
        result = subprocess.run(["lemkin-osint", "--version"],
                              capture_output=True, text=True)
        if result.returncode == 0:
            print(f"✅ CLI lemkin-osint available")
            print(f"   Output: {result.stdout.strip()}")
            return True
        else:
            print(f"❌ CLI lemkin-osint failed with code {result.returncode}")
            return False
    except FileNotFoundError:
        print(f"❌ CLI lemkin-osint not found")
        return False
...
def main():
    ...
    print("🔍 Validating Osint installation...")
    print("=" * 50)
    ...
    if passed == total:
        print("🎉 All validation checks passed! Osint is ready to use.")
        return 0
    else:
