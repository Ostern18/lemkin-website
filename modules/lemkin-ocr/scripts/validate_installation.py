#!/usr/bin/env python3
"""
Validation script for lemkin-ocr
"""
...
def check_module_import():
    ...
    try:
        import lemkin_ocr
        print(f"✅ Module lemkin-ocr imported successfully")
        print(f"   Version: {lemkin_ocr.__version__}")
        return True
    except ImportError as e:
        print(f"❌ Failed to import lemkin-ocr: {e}")
        return False

def check_cli_available():
    """Check if CLI is available"""
    try:
        result = subprocess.run(["lemkin-ocr", "--version"],
                              capture_output=True, text=True)
        if result.returncode == 0:
            print(f"✅ CLI lemkin-ocr available")
            print(f"   Output: {result.stdout.strip()}")
            return True
        else:
            print(f"❌ CLI lemkin-ocr failed with code {result.returncode}")
            return False
    except FileNotFoundError:
        print(f"❌ CLI lemkin-ocr not found")
        return False

def check_core_functionality():
    """Check core functionality"""
    try:
        from lemkin_ocr.core import *
...
def main():
    ...
    print("🔍 Validating Ocr installation...")
    print("=" * 50)
    ...
    if passed == total:
        print("🎉 All validation checks passed! Ocr is ready to use.")
        return 0
    else:
