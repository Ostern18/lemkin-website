#!/usr/bin/env python3
"""
Validation script for lemkin-comms
"""
...
def check_module_import():
    ...
    try:
        import lemkin_comms
        print(f"✅ Module lemkin-comms imported successfully")
        print(f"   Version: {lemkin_comms.__version__}")
        return True
    except ImportError as e:
        print(f"❌ Failed to import lemkin-comms: {e}")
        return False

def check_cli_available():
    """Check if CLI is available"""
    try:
        result = subprocess.run(["lemkin-comms", "--version"],
                              capture_output=True, text=True)
        if result.returncode == 0:
            print(f"✅ CLI lemkin-comms available")
            print(f"   Output: {result.stdout.strip()}")
            return True
        else:
            print(f"❌ CLI lemkin-comms failed with code {result.returncode}")
            return False
    except FileNotFoundError:
        print(f"❌ CLI lemkin-comms not found")
        return False

def check_core_functionality():
    """Check core functionality"""
    try:
        from lemkin_comms.core import *
...
def main():
    ...
    print("🔍 Validating Comms installation...")
    print("=" * 50)
    ...
    if passed == total:
        print("🎉 All validation checks passed! Comms is ready to use.")
        return 0
    else:
