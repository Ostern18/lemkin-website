#!/usr/bin/env python3
"""
Validation script for lemkin-images
"""
...
def check_module_import():
    ...
    try:
        ...
        print(f"✅ Module lemkin-images imported successfully")
        print(f"   Version: {lemkin_images.__version__}")
        return True
    except ImportError as e:
        print(f"❌ Failed to import lemkin-images: {e}")
        return False

def check_cli_available():
    """Check if CLI is available"""
    try:
        result = subprocess.run(["lemkin-images", "--version"],
                              capture_output=True, text=True)
        if result.returncode == 0:
            print(f"✅ CLI lemkin-images available")
            print(f"   Output: {result.stdout.strip()}")
            return True
        else:
            print(f"❌ CLI lemkin-images failed with code {result.returncode}")
            return False
    except FileNotFoundError:
        print(f"❌ CLI lemkin-images not found")
        return False
...
def main():
    ...
    print("🔍 Validating Images installation...")
    print("=" * 50)
    ...
    if passed == total:
        print("🎉 All validation checks passed! Images is ready to use.")
        return 0
    else:
