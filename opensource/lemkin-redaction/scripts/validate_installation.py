#!/usr/bin/env python3
"""
Validation script for lemkin-redaction
"""
...
def check_module_import():
    ...
    try:
        import lemkin_redaction
        print(f"✅ Module lemkin-redaction imported successfully")
        print(f"   Version: {lemkin_redaction.__version__}")
        return True
    except ImportError as e:
        print(f"❌ Failed to import lemkin-redaction: {e}")
        return False

def check_cli_available():
    """Check if CLI is available"""
    try:
        result = subprocess.run(["lemkin-redaction", "--version"],
                              capture_output=True, text=True)
        if result.returncode == 0:
            print(f"✅ CLI lemkin-redaction available")
            print(f"   Output: {result.stdout.strip()}")
            return True
        else:
            print(f"❌ CLI lemkin-redaction failed with code {result.returncode}")
            return False
    except FileNotFoundError:
        print(f"❌ CLI lemkin-redaction not found")
        return False
...
def main():
    ...
    print("🔍 Validating Redaction installation...")
    print("=" * 50)
    ...
    if passed == total:
        print("🎉 All validation checks passed! Redaction is ready to use.")
        return 0
    else:
