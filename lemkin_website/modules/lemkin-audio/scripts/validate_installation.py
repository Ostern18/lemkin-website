#!/usr/bin/env python3
"""
Validation script for lemkin-audio
"""
...
def check_module_import():
    ...
    try:
        import lemkin_audio
        print(f"✅ Module lemkin-audio imported successfully")
        print(f"   Version: {lemkin_audio.__version__}")
        return True
    except ImportError as e:
        print(f"❌ Failed to import lemkin-audio: {e}")
        return False

def check_cli_available():
    """Check if CLI is available"""
    try:
        result = subprocess.run(["lemkin-audio", "--version"],
                              capture_output=True, text=True)
        if result.returncode == 0:
            print(f"✅ CLI lemkin-audio available")
            print(f"   Output: {result.stdout.strip()}")
            return True
        else:
            print(f"❌ CLI lemkin-audio failed with code {result.returncode}")
            return False
    except FileNotFoundError:
        print(f"❌ CLI lemkin-audio not found")
        return False
...
def main():
    ...
    print("🔍 Validating Audio installation...")
    print("=" * 50)
    ...
    if passed == total:
        print("🎉 All validation checks passed! Audio is ready to use.")
        return 0
    else:
